...
export default class Footprint {
  ...
  displayDomInfo() {
    ...
    let outLenAreaRatio = qry("#out-len-area-ratio");
    ...
    outLenAreaRatio.value = this.attr.lenAreaRatio;
  ...
  buildFootprint() {
    ...
    for (let i = 0; i < this.gene.genLen; i++) {
      //setFootprintMatrixPrint from grid2.js
    ...
    let validFootprintMatrix = matrix(this.matFootprint).and(
      matrix(grid.matCellInside)
    );
  ...
  calcLandAttributes() {
    ...
    let area = validCellCount * CELL_SIZE * CELL_SIZE;
    this.attr.area = area;
    this.attr.faRatio =
      Math.round(
        (floorAreaRatio(parcel.area, this.attr.area) + Number.EPSILON) * 10000
      ) / 100;
    let length = matrixPerimeter(this.validFootprintMatrix) * CELL_SIZE;
    this.attr.boundaryLength = length;
    this.attr.lenAreaRatio = lenAreaRatio(length, area);
  }
  ...
  setNextCellActive(cell, iter) {
    ...
    switch (cell.nitrobasis) {
      case "00": //East
        ...
        while (arr2d(col, row) && col < grid.cols - 1) {
          //if grid.arr2d[col][row] aleady exists and not hit the wall, go to one cell more east direction
          col += 1;
        }
        break;
      case "10": //West
