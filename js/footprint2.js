...
export default class Footprint {
  ...
  displayDomInfo() {
    console.log(`displayDomInfo, this.id`, displayDomInfo, this.id);
  ...
  buildFootprint() {
    ...
    for (let i = 0; i < this.dna.dnaLen; i++) {
  ...
  calculateFitness() {
    ...
    this.fitness.faRatio =
      Math.round(
        (floorAreaRatio(parcel.area, this.fitness.area) + Number.EPSILON) *
          10000
      ) / 100;
    this.fitness.boundaryLength =
      matrixPerimeter(this.validFootprintMatrix) * 30;
  }
